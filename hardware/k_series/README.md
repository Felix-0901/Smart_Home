...
- 目前開發模式：主程式暫時不連 WiFi、不上傳後端，只透過 Serial 讀取 Sensor 並用 RGB 顯示狀態，做法對齊 R / T 系列整合測試流程。
...
| MQ 類比輸入 | `IO33` | `GPIO33` | ADC1，WiFi 使用時較適合讀類比 |
...
| MQ 氣體模組 | `GND` | `GND` | 共地 |
| MQ 氣體模組 | `AO` | `GPIO33` | 若 MQ 用 5V 供電，必須先分壓 |
...
```text
MQ AO -- 10k -- ESP32 GPIO33 -- 20k -- GND
```
...
```text
紅色火焰警示 > 黃色氣體警示 > 紫色 Sensor 異常 > 綠色正常
```

目前主程式不檢查 WiFi / API / PostgreSQL，因此青色與後端失敗白色狀態暫不使用；白色只會在 RGB 開機自測中出現。
...
K 系列 MQ 正常時曾讀到約 `300-500`。若封裝後 `gas_raw` 連續為 `0`，通常代表目前 MQ `AO -> IO33` 線材、接觸點或麵包板導通異常，即使 `DO` 沒觸發也不能視為正常。
...
新版韌體已開始上傳 `gas_analog_ok` 欄位。若 `gas_raw == 0`，狀態燈會顯示紫色；低但非零的 `gas_raw` 不會再被判定為 MQ 類比異常。

2026-05-15 封蓋燈號異常診斷：

```text
temperature_c=27.2-27.4
humidity_percent=52.1-52.2
eco2_ppm=499-524
tvoc_ppb=182-262
gas_raw=0
gas_analog_ok=false
...
封蓋後資料仍持續寫入，DHT22、GY-SGP30、火焰 / 紅外線模組與 WiFi 上傳正常。RGB 不是綠色的原因是 MQ 類比輸入 `gas_raw` 連續為 `0`，主程式依規則判定 `gas_analog_ok=false`，狀態燈應顯示紫色 Sensor 異常。打開蓋子後恢復綠燈，代表 MQ 類比線很可能被蓋子、線材角度或接觸壓力影響；目前接線已改為 `AO -> IO33`。

目前 K 系列主程式採本機 Sensor 測試模式，不需要 WiFi / API 設定，會直接在 Serial 輸出每次讀值並依 Sensor 狀態控制 RGB。若 RGB 完全不亮，先確認：
...
此輪測試使用不連 WiFi / 不上傳後端的本機 Sensor 測試韌體。MQ 類比、MQ 數位、GY-SGP30、火焰 / 紅外線模組皆可讀取；DHT22 連續回傳 `null`，因此 RGB 狀態為紫色 Sensor 異常。需檢查 DHT22 `VCC`、`GND`、`DATA -> IO4` 與模組接觸。
...
目前開發階段先暫停 WiFi / 後端上傳，主程式 `hardware/k_series/k_series.ino` 已改為不載入 `config.h`、不連 WiFi、不送 API，只做本機 Sensor 讀取。需要恢復後端上傳時，再依下列設定建立 `config.h` 並把上傳邏輯接回主程式。
...
DHT22、GY-SGP30、火焰 / 紅外線模組、WiFi 與 PostgreSQL 寫入正常。MQ 數位輸出仍為未觸發，但 MQ 類比值 `gas_raw` 連續為 `0`，和先前正常讀值約 `300-500` 不一致；封裝前需檢查 MQ 類比線材、接觸點與麵包板導通。目前接線已改為 `AO -> IO33`。
...
目前本機 Sensor 測試模式正常情況會看到：

- `K series local sensor test`
- `SGP30 available: yes`
- 每約 2 秒輸出一行 DHT22、MQ、GY-SGP30、火焰 / 紅外線模組讀值
- `rgb_status=green_normal` 代表目前 Sensor 讀取正常且未觸發警示

2026-06-01 起，主程式已改回正式 HTTP readings 韌體：

```text
hardware/k_series/k_series.ino
```

此韌體沒有 `config.h` 時仍會讀取 Sensor 並輸出 Serial JSON；存在 `config.h` 時，會連 WiFi 並上傳到：
...
- 2026-06-01：更新 `k_series.ino` 為正式 HTTP readings 韌體，補上 `DEVICE_ID`、標準 `metadata`、MQ 欄位命名、RGB 狀態燈優先順序與 optional `config.h`；尚未進行硬體實測。
...
- 2026-05-15：封裝前再次確認時 DHT22、GY-SGP30、火焰 / 紅外線模組與 WiFi 上傳正常，但 MQ 類比值 `gas_raw` 連續為 `0`，需檢查 MQ 類比接線後再封裝。
- 2026-05-15：補完整 RGB 狀態燈規劃與優先順序，新增白色代表後端上傳失敗，並將 MQ `gas_raw == 0` 納入紫色 Sensor 異常判斷，避免 MQ 類比線斷線或無讀值時仍顯示綠色。
- 2026-05-15：燒錄新版 RGB 狀態燈規則後，資料庫確認新版 payload 已包含 `gas_analog_ok=true`，最新 MQ `gas_raw=10-20`，未達異常門檻。
- 2026-05-15：封蓋燈號異常診斷確認 DHT22、GY-SGP30、火焰 / 紅外線模組與 WiFi 正常，但 MQ `gas_raw=0`、`gas_analog_ok=false`，需檢查 MQ 類比線是否受蓋子或線材角度影響。
- 2026-05-15：依使用者要求將 MQ 類比異常條件從 `gas_raw <= 5` 改為只有 `gas_raw == 0` 才觸發，避免低但非零的類比讀值被誤判為異常。
- 2026-05-15：燒錄新版 `gas_raw == 0` 判斷後查詢 PostgreSQL，確認 `gas_raw=0` 時 `gas_analog_ok=false`，後續 `gas_raw=190-534` 時 `gas_analog_ok=true`，新規則已生效。
- 2026-05-15：依使用者要求將 MQ 類比輸入從 `IO34` / `GPIO34` 改為 `IO33` / `GPIO33`，避免繼續使用封蓋時容易受線材角度影響的位置；主程式同步改為讀取 `GPIO33`。
- 2026-05-15：燒錄 `MQ AO -> IO33` 版本後查詢 PostgreSQL，曾讀到 `gas_raw=99`、`143`、`4` 且 `gas_analog_ok=true`，確認新版腳位可讀到 MQ 類比值；後續仍曾回到 `gas_raw=0`、`gas_analog_ok=false`，表示 MQ 實體接線或模組輸出仍需再固定確認。
- 2026-05-30：依目前 K 系列開發需求，主程式改為本機 Sensor 測試模式，效仿 R / T 系列整合測試：移除 WiFi / HTTP / `config.h` 依賴，不上傳後端，只透過 Serial 輸出 DHT22、GY-SGP30、MQ、火焰 / 紅外線模組讀值，並以 RGB 顯示狀態。
- 2026-05-30：同步修正 K 系列 `tests/integration_test/`：MQ 類比腳改為 `IO33` / `GPIO33`，新增 `gas_analog_ok` 判斷，並補上 Arduino ESP32 core 3.x 的 RGB PWM 相容寫法。
- 2026-05-30：燒錄 K 系列本機 Sensor 測試韌體並直接讀取 Serial；MQ、GY-SGP30、火焰 / 紅外線模組正常，DHT22 連續讀取失敗，狀態為 `rgb_status=purple_sensor_error`。
- 2026-05-15：依照 `LIVE MINI KIT ESP32` 背面絲印修正腳位配置，避免使用未外露或不適合的 GPIO。
- 2026-05-15：確認 MQ 氣體模組舊接線 `AO -> IO34`、`DO -> IO25` 測試通過；後續已將 `AO` 改接 `IO33`。
