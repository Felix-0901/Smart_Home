...
const char *SERIES_KEY = "t_series";
...
void waitAndSamplePir(unsigned long durationMs) {
  const unsigned long startedAt = millis();
  while (millis() - startedAt < durationMs) {
    if (millis() - lastPirSampleAt >= PIR_SAMPLE_INTERVAL_MS) {
      samplePir();
      lastPirSampleAt = millis();
    }
    delay(1);
  }
}
...
String rgbStatusFor(bool gasDetected, bool sensorError, bool motionDetected, bool networkOk, bool uploadOk) {
  if (gasDetected) {
    setRgb(255, 180, 0);
...
void sendReading() {
  ...
  samplePir();
  const int pirLevel = digitalRead(PIR_PIN);
  const bool motionDetected = motionLatched || pirLevel == HIGH;
  ...
  const String preStatus = rgbStatusFor(gasDetected, sensorError, motionDetected, networkOk, true);
  ...
  json += pirLevel;
  json += ",";
  appendBool(json, "motion_detected", motionDetected);
  json += ",";
  appendBool(json, "motion_latched", motionLatched);
  json += ",\"wifi_rssi\":";
  ...
  json += "\",\"device_id\":\"";
  json += DEVICE_ID;
  json += "\",\"board\":\"esp32_d0wd_v3\",\"firmware_version\":\"";
  json += FIRMWARE_VERSION;
  json += "\",\"transport\":\"http\",\"source\":\"firmware\"}}";

  Serial.println(json);
  const bool uploadOk = postPayload(json);
  const String finalStatus = rgbStatusFor(gasDetected, sensorError, motionDetected, networkOk, uploadOk);
...
void setup() {
  ...
  Serial.println();
  Serial.println("T series firmware");
