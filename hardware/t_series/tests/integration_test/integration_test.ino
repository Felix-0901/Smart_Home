...
void setup() {
  ...
  Serial.println();
  Serial.println("T series integration test");
  Serial.println("RGB status: green=normal, yellow=gas DO warning, blue=motion, purple=sensor error");
...
void loop() {
  ...
  if (gasDetected) {
    ...
    setRgb(0, 0, 255);
    rgbStatus = "blue_motion_detected";
  } else {
    setRgb(0, 255, 0);
  ...
  waitAndSamplePir(LOOP_INTERVAL_MS);
}
