...
void setup() {
  ...
  Serial.println();
  Serial.println("T series MQ gas sensor test");
  Serial.println("Expected wiring:");
  Serial.println("  MQ VCC -> 5V/VIN or 3V3 depending on module");
  Serial.println("  MQ GND -> GND");
  Serial.println("  MQ AO  -> GPIO33, use voltage divider if MQ is powered by 5V");
  Serial.println("  MQ DO  -> GPIO25 only if output is 3.3V-safe");
