...
`app/` 是 Smart Home 專題的行動應用程式，使用 React Native / Expo 建立。第一版提供註冊登入、產品編號綁定裝置、展示邀請碼兌換、房屋與空間管理、P 系列智慧插座控制、即時資料查看、歷史資料查詢、Homi Agent 與個人資料管理。
...
個人：輸入展示邀請碼、切換依系列/依空間顯示、開關開發者模式、導向帳號或房屋管理
房屋：新增/重新命名房屋、新增/重新命名空間、進入指定房屋
```

Homi 不會直接操作刪除帳號、修改密碼、刪除房屋、刪除空間或刪除裝置等高風險功能。展示邀請碼也由使用者自行輸入兌換，Homi 只負責導覽到入口。這些功能只會導頁或提醒使用者自行確認。
...
產出目錄為 `dist/`，此目錄不提交版本控制。

## 部署細節

目前 APP 可用兩種方式部署：

- 網頁版：使用 `app/Dockerfile` 建置 Expo Web 靜態檔，Nginx 對外提供網站。
- 原生版：若後續要發佈到 TestFlight 或正式商店，需再補 EAS Build 設定、bundle identifier、app icon、splash、隱私權文字與正式 API domain。
...
`EXPO_PUBLIC_API_BASE_URL` 會在 `npm run build:web` 時寫入前端 bundle。Coolify 變更此值後，需要重新 build APP。

Coolify 使用 Dockerfile 部署 APP Web 時：
...
Build Context: app
Port: 80
Build Variable: EXPO_PUBLIC_API_BASE_URL=https://你的後端網域
```
