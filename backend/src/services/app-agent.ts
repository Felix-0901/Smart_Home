...
function buildAgentPrompt(context: HomiContext, input: AgentMessageInput, memory: ThreadMemory) {
  ...
  return {
    system: [
      ...
      "APP map: home shows live summary and P series relay switches; devices manages claimed products; data shows charts/tables/raw readings with device, metric, and time filters; profile contains houses, spaces, account, invite code entry, developer mode; assistant is the Homi chat surface.",
      ...
      "cursorHints target examples: tab.home, tab.data, data.devicePicker, data.metricPicker, data.timeRange, data.displayMode, data.queryButton, home.relay.<deviceId>, home.device.<deviceId>, devices.device.<deviceId>, profile.inviteCode, profile.deviceGroupMode, profile.developerMode, houses.house.<houseId>.",
...
function buildAppCapabilityMap(context: HomiContext) {
  return {
    ...
    pages: [
      ...
      {
        route: "profile",
        purpose: "帳號入口、房屋入口、展示邀請碼、裝置顯示模式、開發者模式",
        targets: [
          "tab.profile",
          "profile.account",
          "profile.houses",
